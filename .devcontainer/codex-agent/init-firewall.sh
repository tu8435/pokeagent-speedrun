#!/bin/bash
# Firewall initialization for Codex CLI agent container.
# Runs as root (via sudo) before handing off to the codex-agent user.
#
# Network policy (same as Claude/Gemini):
#   ALLOW  loopback
#   ALLOW  DNS (UDP/TCP 53)
#   ALLOW  HTTPS (TCP 443) — OpenRouter, OpenAI API
...
cat > /tmp/run_codex.sh << 'WRAPPER_EOF'
#!/bin/sh
export HOME=/home/codex-agent
export CODEX_HOME=/home/codex-agent/.codex
exec "$@"
WRAPPER_EOF
...
if [ "${SKIP_FIREWALL:-0}" = "1" ]; then
    ...
    exec su codex-agent -c "/tmp/run_codex.sh $*"
fi
...
iptables -A OUTPUT -p tcp --dport 443 -j ACCEPT   # HTTPS (OpenRouter, OpenAI)
...
# Switch to codex-agent user and exec the agent command.
exec su codex-agent -c "/tmp/run_codex.sh $*"
